...
print()
print('PASS 1.5: correct CDE data, remove 0.4A and 27-30VA on L3...')
for (m, meter_id) in enumerate(meter_ids):
    if meter_id == 'CDE':
        err_a = 0.4
        err_s = 28
        ...
        for i in range(len(meter_data[meter_id])):
            if meter_data[meter_id][i] == []:                
                #x = input('\tFount empty row at %d adding data...' % (i))
                print('\tFount empty row at %d adding data... ' % (i), end = ' ')
                
                meter_data[meter_id][i] = meter_data[meter_id][i-1][:]                    
                meter_data[meter_id][i][0] = '+' + str(start_ts + i * 60)
                empty_rows += 1
                print('(I,P,Pt,Q,Qt,S,St): %4.1f, %5d, %7d, %5d, %7d, %5d, %7d' % (meter_data[meter_id][i][2], meter_data[meter_id][i][6], meter_data[meter_id][i][7], meter_data[meter_id][i][8], meter_data[meter_id][i][9], meter_data[meter_id][i][10], meter_data[meter_id][i][11]))
                continue
            
            print('\t(I,P,Pt,Q,Qt,S,St): %4.1f, %5d, %7d, %5d, %7d, %5d, %7d -->' % (meter_data[meter_id][i][2], meter_data[meter_id][i][6], meter_data[meter_id][i][7], meter_data[meter_id][i][8], meter_data[meter_id][i][9], meter_data[meter_id][i][10], meter_data[meter_id][i][11]), end = ' ')    
            
            if meter_data[meter_id][i][2] == err_a:
                print('POINT4:', end = ' ')
                meter_data[meter_id][i][2] = 0.0
                meter_data[meter_id][i][6] = 0
                meter_data[meter_id][i][7] = meter_data[meter_id][i-1][7] if i > 1 else meter_data[meter_id][i][7]
                meter_data[meter_id][i][8] = 0
                meter_data[meter_id][i][9] = meter_data[meter_id][i-1][9] if i > 1 else meter_data[meter_id][i][9]
                meter_data[meter_id][i][10] = 0
                meter_data[meter_id][i][11] = meter_data[meter_id][i-1][11] if i > 1 else meter_data[meter_id][i][11]
                point4_change += 1
                first_on = True
                diff_pt = 0
                diff_qt = 0
                ...
                print('ACTIVE:', end = ' ')
                if first_on:
                    diff_pt = meter_data[meter_id][i][7] - meter_data[meter_id][i-1][7]
                    diff_qt = meter_data[meter_id][i][9] - meter_data[meter_id][i-1][9]
                    diff_st = meter_data[meter_id][i][11] - meter_data[meter_id][i-1][11]
                    first_on = False
                
                meter_data[meter_id][i][2] = round(meter_data[meter_id][i][2] - err_a, 1)
                #meter_data[meter_id][i][6] = 0
                meter_data[meter_id][i][7] -= diff_pt
                #meter_data[meter_id][i][8] = 0
                meter_data[meter_id][i][9] -= diff_qt
                meter_data[meter_id][i][10] -= err_s
                meter_data[meter_id][i][11] -= diff_st + (count % 2)
                active_change += 1
                count += 1
                

            print('%4.1f, %5d, %7d, %5d, %7d, %5d, %7d' % (meter_data[meter_id][i][2], meter_data[meter_id][i][6], meter_data[meter_id][i][7], meter_data[meter_id][i][8], meter_data[meter_id][i][9], meter_data[meter_id][i][10], meter_data[meter_id][i][11]))    

            if i - 1 in [797429, 797437, 797440]:
                delta_pt = meter_data[meter_id][i][7] - meter_data[meter_id][i-1][7]
                delta_qt = meter_data[meter_id][i][9] - meter_data[meter_id][i-1][9]
                delta_st = meter_data[meter_id][i][11] - meter_data[meter_id][i-1][11]
                
                if i == 797437:
                    meter_data[meter_id][i-1] = meter_data[meter_id][i][:]
                
                percent = 0.50
                if i == 797440:
                    percent = 0.90    
                    
                meter_data[meter_id][i-1][7] += int(delta_pt * percent)
                meter_data[meter_id][i-1][9] += int(delta_qt * percent)
                meter_data[meter_id][i-1][11] += int(delta_st * percent)
                
                print('\tCorrected i-1 (Pt,Qt,St): %7d, %7d, %7d' % (meter_data[meter_id][i-1][7], meter_data[meter_id][i-1][9], meter_data[meter_id][i-1][11]))

        print('\tChange Stats: point4_change =', point4_change, ', active_change =', active_change, ', empty_rows =', empty_rows)        
...
for tick in range(minute_len):
    ...
    for i, meter_id in enumerate(meter_ids):
        if meter_data[meter_id][tick] == []:
            ...
            if tick > 0 and meter_data[meter_id][tick-1][0][:1] == '+' and meter_data[meter_id][tick-1][11] == '0':
                hist = 0
